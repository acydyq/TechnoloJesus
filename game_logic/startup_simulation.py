...
class Employee:
    def __init__(self, name, role, skill_level):
        """Initializes an employee with attributes, enthusiasm, performance tracking, and salary."""
        self.name = name
        self.role = role
        ...
        # Default game salaries (not based on real-world averages)
        base_salary = 1000  # Base salary for all employees
        self.salary = base_salary * skill_level  # Adjust salary based on skill level
    ...
    def __repr__(self):
        ...
        return f"{self.name} ({self.role}) - Skill: {self.skill_level} | Salary: ${self.salary} | Enthusiasm: {self.enthusiasm} | Happiness: {self.happiness} | Loyalty: {self.loyalty} | {status}"
...
class EmployeeManager:
    ...
    def calculate_total_salary(self):
        """Returns the total salary expenses per cycle."""
